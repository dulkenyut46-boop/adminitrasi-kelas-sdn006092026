...
import { syncStudent, syncTeacher, syncGrade, syncTransaction, syncInventoryItem } from '../../services/firestoreSync';
...
export const ImportExcelView: React.FC = () => {
  ...
  const handleSaveImportedData = async () => {
    ...
    try {
      if (activeCategory === 'siswa') {
        const savedStudents = localStorage.getItem('admin_kelas_sd_v1_students');
        let currentList: Student[] = savedStudents ? JSON.parse(savedStudents) : students;

        if (importMode === 'replace') {
          currentList = parsedData as Student[];
        } else {
          // Merge by NISN / NIS
          const map = new Map<string, Student>();
          currentList.forEach(s => map.set(s.nisn || s.id, s));
          (parsedData as Student[]).forEach(s => map.set(s.nisn || s.id, { ...(map.get(s.nisn || s.id) || {}), ...s }));
          currentList = Array.from(map.values());
        }

        // Save to localStorage & sync
        localStorage.setItem('admin_kelas_sd_v1_students', JSON.stringify(currentList));
        for (const s of parsedData) {
          syncStudent(s).catch(console.error);
        }
      } else if (activeCategory === 'guru') {
      ...
      addToast(
        ...
        `Sebanyak ${parsedData.length} data ${activeCategory.toUpperCase()} telah berhasil diimpor ke database.`
      );
      ...
      // Reload window gently or let reactive state trigger
      setTimeout(() => {
